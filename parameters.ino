...
void resetParameters() {
	// storage.clear();
	// ESP.restart();
	storage.begin("flix", false); // 打开命名空间 
	storage.clear(); // 清空命名空间 
	storage.end(); // 确保写入完成 
	Serial.println("参数存储已清空，重启后将使用代码默认值");
	ESP.restart(); // 重启设备
}

void dumpParameters() {   //存储中的变量
    storage.begin("flix", false);   // 打开命名空间
    Serial.println("=== 存储参数列表 ===");
    for (auto &parameter : parameters) {
        float storedValue = storage.getFloat(parameter.name, *parameter.variable);
        Serial.printf("%s : 存储值 = %f, 当前值 = %f\n",
                      parameter.name,
                      storedValue,
                      *parameter.variable);
    }
